...
def get_urls(url:str, categories: list, no_of_urls:int, output_file="urls.txt"):
    no_of_items = f"?start=0&sz={no_of_urls}&format=page-element"
    f = open(output_file, "w")
    for category in categories:
        url = url + category + no_of_items
        request = Request(url, headers=headers)
        page = urlopen(request)
        html = page.read().decode("utf-8")
        soup = BeautifulSoup(html, "html.parser")
        products = soup.find_all("a", class_="product-badge-link", href=True)
        for product in products:
            f.write(f"{product['href']}\n")
    f.close()


if __name__ == "__main__":
    website = "https://www.versace.com/eu/en/"
    categories = ["women/accessories/"]
    get_urls(website, categories, no_of_urls=350)
